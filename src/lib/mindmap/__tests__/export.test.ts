...
describe("computeLayout", () => {
  ...
  it("skips children of collapsed nodes so a collapsed branch is not exported", () => {
    const layout = computeLayout(sample.nodes, new Set(["c1"]));
    expect(layout.nodes.map((n) => n.id)).not.toContain("d1");
    expect(layout.nodes.map((n) => n.id)).toContain("c2");
  });
});
